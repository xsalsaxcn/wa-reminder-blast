...
function getPaidRate() {
  return Number(process.env.WA_ESTIMATED_PAID_MESSAGE_COST_IDR || 45)
}
...
function buildBillingInfo({ status, sentAt, phone, incomingByPhone }) {
  ...
  if (!['sent', 'delivered', 'read'].includes(String(status || '').toLowerCase())) {
    return {
...
export async function buildUsageRows(filters = {}) {
  ...
  const summary = {
    ...
    sent: filteredRows.filter((row) => ['sent', 'delivered', 'read'].includes(String(row.status || '').toLowerCase())).length,
    ...
    outsideWindow: filteredRows.filter(
      (row) => ['sent', 'delivered', 'read'].includes(String(row.status || '').toLowerCase()) && !row.is_24h_window
    ).length,
