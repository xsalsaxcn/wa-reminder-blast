...
export default function MetaTemplatesPage() {
  ...
  async function loadTemplates(sync = true) {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        throw new Error(getErrorMessage(data, 'Gagal memuat templates.'))
      }
  ...
  async function submitTemplate(event) {
    ...
    try {
      if (bodyLength > 1024) {
        throw new Error('Body template maksimal 1.024 karakter. Pendekkan body dulu.')
      }
      ...
      if (!response.ok || !data.success) {
        throw new Error(getErrorMessage(data, 'Gagal submit template ke Meta.'))
      }
  ...
  return (
    <div className="min-h-screen bg-slate-50 lg:flex text-slate-950">
      <Sidebar />
              ...
              <div className="mt-5 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="blast_cv26_interest"
                ...
                {form.header_type !== 'NONE' ? (
                  <div className="rounded-2xl border border-amber-100 bg-amber-50 p-4">
                    ...
                    <p className="mt-1 text-xs text-amber-700">
                      File ini untuk contoh review template Meta. Saat blast, file per kontak bisa diganti dengan attachment_url.
                    </p>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-cyan-400"
                  />
                  <p className={`mt-1 text-xs ${bodyLength > 1024 ? 'text-rose-600 font-bold' : 'text-slate-500'}`}>
                    Variable terdeteksi: {variableCount}. Karakter: {bodyLength}/1024.
                  </p>
                </div>
                ...
                <div>
                  ...
                  <input
                    value={form.buttons}
                    onChange={(event) => updateForm('buttons', event.target.value)}
                    placeholder="Berminat | Tidak Minat"
                    className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-cyan-400"
                  />
                  <p className="mt-1 text-xs text-slate-500">
                    ...
                    <tbody>
                      {templates.map((template) => (
                        <tr key={template.id || `${template.name}-${template.language}`} className="border-b border-slate-50">
                          ...
                          <td className="max-w-xs px-3 py-4 text-slate-600">
                            {getButtonsFromComponents(template.components) || '-'}
                          </td>
                          <td className="max-w-xs px-3 py-4 text-slate-600">
