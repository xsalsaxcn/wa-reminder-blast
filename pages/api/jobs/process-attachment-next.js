...
export default async function handler(req, res) {
  ...
  try {
    ...
    const attachmentCandidates = allPendingItems.filter(hasAttachment)
    const futureItems = attachmentCandidates.filter((item) => isFuture(item, force))
    const attachmentItems = attachmentCandidates.filter((item) => isDue(item, force)).slice(0, safeLimit)

    if (!attachmentItems.length) {
      return res.status(200).json({
        success: true,
        message: futureItems.length
          ? 'Belum ada attachment item yang waktunya due. Semua masih menunggu scheduled_at.'
          : 'Tidak ada attachment item pending untuk diproses.',
    ...
    for (const item of attachmentItems) {
      ...
      const attachmentUrl = cleanText(item.attachment_url)

      affectedJobIds.add(itemJobId)

      if (!itemId || !phone || !attachmentUrl) {
        await updateItemSafe(itemId, {
          status: 'failed',
          processed_at: new Date().toISOString(),
          error_message: 'Phone atau attachment_url kosong.'
        })
        ...
        results.push({
          ...
          phone,
          success: false,
          error: 'Phone atau attachment_url kosong.'
        })
      ...
      try {
        const sendResult = await sendWhatsAppMedia({
          ...
          attachment_type: cleanText(item.attachment_type),
          attachment_filename: cleanText(item.attachment_filename),
          attachment_caption: cleanText(item.attachment_caption) || message
        })
        ...
        await updateItemSafe(itemId, {
          status: 'failed',
          processed_at: new Date().toISOString(),
          error_message: error.message || 'Gagal kirim attachment WhatsApp.'
        })
        ...
        results.push({
          ...
          phone,
          success: false,
          error: error.message || 'Gagal kirim attachment WhatsApp.'
        })
      }
    }
    ...
    return res.status(200).json({
      success: true,
      message: 'Process attachment batch selesai.',
      mode: force ? 'now' : 'scheduled',
      checked: allPendingItems.length,
      future_items: futureItems.length,
      processed: attachmentItems.length,
      sent,
      failed,
      results
    })
    ...
    return res.status(500).json({
      success: false,
      message: error.message || 'Process attachment batch gagal.'
    })
  }
}